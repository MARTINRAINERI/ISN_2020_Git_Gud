...
public class Game extends Canvas implements Runnable{
	...
	public Game() {
		new Window(WIDTH,HEIGHT,"Git Gud",this);
		menu=new Menu(this);
	...
	public void render() {
		...
		if(gameState==STATE.Game) {
			...
			g.drawString("Munitions: "+ammo,240,35);
			

			Font fnt1 = new Font("Courier",1,20);
			g.setFont(fnt1);
			g.setColor(Color.white);
			g.drawString("Temps restant: "+countdown.gettimecounter(),740,35);
		}
		else if (gameState==STATE.Pause){
			pause.render(g);
		}
		...
		for(int xx=0;xx<w;xx++) {
			for(int yy=0;yy<h;yy++) {
				...
				if(red == 255 && blue == 255 && green == 0)
					handler.addObject(new Ghosts(xx*32,yy*32,ID.ghosts,handler, ss));
				if (red == 0 && blue == 255 && green == 255)
					handler.addObject(new Crate(xx*32, yy*32, ID.crate, ss));
				if (red==255 && blue==255 && green==255)
					handler.addObject(new Medikit(xx*32, yy*32, ID.medikit, ss));
				if (red==255 && blue==201 && green==174)
					handler.addObject(new trap(xx*32, yy*32, ID.trap, ss));
				if(red == 0 && blue == 0 && green == 255)
					handler.addObject(new Orcs(xx*32,yy*32,ID.orcs,handler, ss));
			}
		}
	}
